 ...
 */
export function parseDecisionFile(filePath, content) {
 ...
 */
function parseFilesChangedTable(content) {
    const files = [];
    const lines = content.split("\n");
    for (const line of lines) {
 ...
 */
export function importDecisionFiles(db, logDir, options = {}) {
    const { skipSummaries = true, verbose = false } = options;
    const result = { imported: 0, skipped: [], duplicates: [], errors: [] };
    ...
    for (const file of files) {
        ...
        if (skipSummaries && summaryPatterns.some((p) => p.test(file))) {
            ...
            if (verbose)
                console.log(`Skipped (summary): ${file}`);
            continue;
        }
        try {
 ...
 */
export function runImport(dbPath, logDir) {
