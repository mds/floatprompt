 ...
 */
function hashFolder(children: { path: string; hash: string }[]): string {
...
function walkFileSystem(
  ...
  folders: {
    ...
    name: string;
    children: { path: string; hash: string }[];
  }[];
  files: {
    path: string;
    folder_path: string;
  ...
  function walk(currentPath: string) {
    ...
    const parentPath =
      relPath === "/" ? null : toRelativePath(path.dirname(currentPath), rootPath);

    const children: { path: string; hash: string }[] = [];
...
export function scan(options: ScanOptions): ScanResult {
  ...
  // Walk filesystem
  const { folders, files } = walkFileSystem(
    rootPath,
    rootPath,
    ignoreList,
    includeList,
  ...
  const syncAll = db.transaction(() => {
    ...
    for (const folder of folders) {
      ...
      const existing = getFolder.get(folder.path) as
        | { path: string; source_hash: string }
        | undefined;
    ...
    for (const file of files) {
      ...
      const existing = getFile.get(file.path) as
    ...
    const existingFolders = getAllFolderPaths.all() as { path: string }[];
    ...
    const existingFiles = getAllFilePaths.all() as { path: string }[];
 ...
 * CLI entry point.
 */
export function runScan(rootPath: string, dbPath: string): void {
