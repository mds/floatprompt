...
- **Phase 1** — Flat files for prototyping (easy to see/edit) ✅
- **Phase 2** — SQLite importer + exporter ✅
- **Phase 3** — Scanner (filesystem → folders/files tables) ✅
- **Phase 4 (next)** — AI generation (buoys populate map_*/context_*)
...
Only read these if you need deeper context (paths relative to `.float-wip/_wip/`):

| File | When to read |
|------|--------------|
| `wip-phase4-qa.md` | **Phase 4 QA — Open decisions (summaries in DB, scopes, export)** |
| `wip-phase3.md` | **Phase 3 planning — COMPLETE (scanner built, verified)** |
...
1. **Moved `docs/sys/` → `.float-wip/`** — Manual prototype at root
2. **Renamed `.float/` → `.float-old/`** — Clear stale signal
3. **Restructured `.float-wip/`** — `_wip/` for session materials, `project/_project/` for root context
...
17. **Imported Test Data**
    - 12 log entries imported to `.float-wip/float.db`
...
### 2026-01-03 (Phase 3 Planning Complete)

20. **Phase 3 Planning Complete**
    - Created `wip-phase3.md` with 15 questions
    - Answered all 15 questions (11 locked, 4 working theories)
    - Documented the 3-layer architecture (mechanical/AI/ongoing)
    - Autonomous scopes concept documented
    - Ready to build `src/db/scan.ts`

21. **Key Decisions Locked**
    - Scan parent of `.float/`, hardcode `.float-wip/` for prototype
    - Ignore: node_modules, .git, dist, build, out, coverage, hidden (except .float*, .claude/)
    - Recursive, no depth limit
    - Populate both `folders` AND `files` tables
    - SHA-256 for content hashing
    - Two-phase change detection: mtime first, then hash
    - Skip symlinks
    - Remove deleted folders/files from DB

22. **Working Theories**
    - Leave `map_*` and `context_*` NULL for Layer 2 (AI)
    - Both layers run in sequence during `/float`
    - API: `scan(db, options) → ScanResult`

### 2026-01-03 (Phase 3 Implementation Complete)

23. **Built Scanner**
    - Created `src/db/scan.ts` — filesystem scanner
    - Created `.float/float.db` — project context database
    - 65 folders, 446 files scanned from repo

24. **Verified**
    - Folder hierarchy integrity (no orphans)
    - File-folder integrity (no orphans)
    - Ignore patterns working (.float, .git, node_modules)
    - Allowed hidden working (.claude)
    - Idempotency (re-run = 0 changes)
    - Change detection (add/remove files)
    - Hash bubbling (folder hashes update)
    - TypeScript build (compiles cleanly)

### 2026-01-03 (Schema Fix)

25. **Found Schema Drift**
    - `files` table missing `mtime` column in actual database
    - Schema.ts defined it, but CREATE TABLE IF NOT EXISTS didn't add it
    - scan.ts collected mtime but INSERT/UPDATE didn't use it

26. **Fixed Schema**
    - Dropped `files` table
    - Updated `scan.ts` — added mtime to INSERT and UPDATE statements
    - Rebuilt TypeScript, rescanned
    - 446 files now have proper mtime values

27. **Verified**
    - PRAGMA table_info shows mtime column exists
    - All 446 files have non-NULL mtime
    - Schema now matches Zod definition
...
└── scan.ts      # Filesystem scanner (mtime fix applied)
```

### 2026-01-03 (Production Cleanup)

28. **Cleaned src/db/ Files**
    - Removed all WIP doc references (wip-phase3.md, wip-logs.md)
    - Updated CLI defaults to `.float/` paths
    - Comments now self-documenting, no external doc references
    - Removed `.float-manual` and `.float-old` from ignore list (dev-only artifacts)

29. **Renamed .float-manual → .float-wip**
    - Clearer naming: "work in progress" not "manual"
    - Matches wip-* prefix convention
    - Updated all references in .float-wip/ markdown files
    - Rebuilt dist/ to clear stale references

30. **Added export.ts**
    - SQLite → Markdown exporter (reverse of import.ts)
    - Creates `.float/logs/` from database
    - Roundtrip verified: import → export → import works

31. **Parity Check Complete**
    - All 5 src/db/ files consistent with each other
    - Schema matches vision (except autonomous scopes - future)
    - No WIP references remain in production code

32. **Created Phase 4 QA Doc**
    - Captured open decisions: summaries in DB, scopes, export completeness
    - Must resolve before Layer 2 (AI generation)
...
**Phase 3 complete.** Scanner built, verified, 65 folders + 446 files in `.float/float.db`.

**Production cleanup complete.** All src/db/ files are production-ready.

**Next:** Phase 4 QA — resolve open decisions (summaries in DB, scopes, export), then Layer 2

---

*Created 2026-01-02 — Updated 2026-01-03 with schema fix (mtime column)*
</md>
</fp>
