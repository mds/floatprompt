...
Manual prototype lives at `.float-wip/`, not `docs/sys/`.

## Rationale

| Factor | `docs/sys/` | `.float-wip/` |
|--------|-------------|------------------|
...
- Create `project/_/` structure inside `.float-wip/`
...
**For future AI sessions:** When you see `docs/sys/` in archive files, it now means `.float-wip/`. Do not "fix" archive references — document the mapping in logs instead.
