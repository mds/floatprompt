...
All tools in `.float/tools/float-*.md` must be at the same version.

```bash
# Quick check
grep -h '"version"' .float/tools/float-*.md | sort -u
```
...
grep -l "## Duality" .float/tools/float-*.md | wc -l
grep -l "## Examples" .float/tools/float-*.md | wc -l
grep -l "invisible OS for AI" .float/tools/float-*.md | wc -l
```

### 3. Command Sync

Each tool in `.float/tools/` must have a corresponding command wrapper in `.claude/commands/`.
...
```bash
# List actual tools
ls .float/tools/float*.md | xargs -n1 basename
...
diff .float/tools/ templates/.float/tools/
```
...
```bash
# Version check
grep -h '"version"' .float/tools/float*.md | sort -u | wc -l
